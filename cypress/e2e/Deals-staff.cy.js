...
describe('Deals Test', () => {
  ...
  it('Click on view details', () => {
    // Ensure 'Deals' button is visible and click it
    cy.get('div').contains('Deals').should('be.visible').click();
  
    // Use XPath to find the specific row and button, wait until it's visible
    cy.xpath("//tr[@data-testid='MUIDataTableBodyRow-0']//button[@type='button']").click();
  
    // Ensure the "View details" link is visible and click it
    cy.xpath("//a").contains("View details").click();
  
    // Wait for any async actions (optional if necessary)
    cy.wait(5000);
  });
